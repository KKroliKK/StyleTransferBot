...
start_inline = InlineKeyboardMarkup(row_width=1)\
					  	.add(InlineKeyboardButton(text=mes.see_example, 
...
style_inline = InlineKeyboardMarkup(row_width=1)\
			       .add(InlineKeyboardButton(text='Download your own style photo', callback_data='download'),
						InlineKeyboardButton(text='Choose a ready-made style photo', callback_data='choose'),
...
async def choose_style(message: types.Message):
	...
	await bot.send_message(message.from_user.id, mes.style_1, reply_markup=ReplyKeyboardRemove())
...
cancel_inline = inkb = InlineKeyboardMarkup(row_width=1)\
					  	.add(InlineKeyboardButton(text='cancel', 
...
async def offer_send_style(callback: types.CallbackQuery):
	...
	await bot.send_message(callback.from_user.id, mes.download_style, reply_markup=cancel_inline)
...
async def dowload_style_photo(message: types.Message, state: FSMContext):
	...
	await bot.send_message(message.from_user.id, mes.download_content, reply_markup=cancel_inline)
...
async def offer_choose_style(callback: types.CallbackQuery):
	...
	for photo in await style_photos.read2():	
		await bot.send_photo(callback.from_user.id, photo[1],
							 reply_markup=InlineKeyboardMarkup().\
								add(InlineKeyboardButton('Choose', callback_data=f'index: {photo[0]}')))
	await bot.send_message(callback.from_user.id, mes.choose_style, reply_markup=cancel_inline)
...
async def save_choosed_style(callback: types.CallbackQuery, state: FSMContext):
	...
	async with state.proxy() as data:
		data['style'] = (await style_photos.get(callback.data.replace('index: ', '')))[0]
	await bot.send_message(callback.from_user.id, mes.download_content, reply_markup=cancel_inline)
...
async def dowload_content_photo(message: types.Message, state: FSMContext):
	await bot.send_message(message.from_user.id, mes.wait_result, reply_markup=ReplyKeyboardRemove())
	...
	content = BytesIO(requests.get(DOWNLOAD_URL + content.file_path, stream=True).content)
